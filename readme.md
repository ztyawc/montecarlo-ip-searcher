...
正式发布使用 Flutter 客户端。两套 Android 客户端包名相同，不能同时安装；已有版本只有使用同一签名时才能直接覆盖升级。扫描期间需要保持前台。

从 0.4.1 起，Android 会自动保存最近 50 次扫描记录，重开应用可查看结果，新扫描会保留上一轮。在“结果 → 历史记录”中查看、复制结果或删除单条记录；历史浏览与当前扫描互不影响。异常退出后，已保存的检查点显示为“已中断”，不会自动继续扫描。记录仅保存在本机，详见 [历史记录说明](docs/android-history.md)。
